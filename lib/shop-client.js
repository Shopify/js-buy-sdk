...
const ShopClient = CoreObject.extend({
  ...
  fetchAll(type) {
    ...
    return adapter.fetchCollection().then(payload => {
      return this.serialize(type, payload, adapter, { collection: true });
    });
  },
  ...
  fetchOne(type, id) {
    ...
    return adapter.fetchSingle(id).then(payload => {
      return this.serialize(type, payload, adapter, { single: true });
    });
  },
  ...
  serialize(type, payload, adapter, opts = {}) {
    const serializer = new this.serializers[type](this.config);
    const meta = { shopClient: this, adapter, serializer };
    let serializedPayload;

    if (opts.collection) {
      serializedPayload = serializer.serializeCollection(payload, meta);
    } else {
      serializedPayload = serializer.serializeSingle(payload, meta);
    }
